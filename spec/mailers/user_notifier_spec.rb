...
RSpec.describe UserNotifierMailer, type: :mailer do
  describe 'registration_email' do
    let(:user) { mock_model User, first_name: "Lucas", email: "lucas@email.com", temporary_token: "12345678" }
    let(:mail) { UserNotifierMailer.inform(user.email, user.temporary_token).deliver_now }
